...
public class HolidayAutoConfiguration implements InitializingBean {
  ...
  private void checkConfigFileExists() {
    if (StringUtils.hasText(this.properties.getType())
        && "local".equals(this.properties.getType())) {
      ...
      Assert.state(
          ...
          "Cannot find config location: "
              + resource
              + " (please add config file or check your Holiday configuration)");
      Assert.state(
          Objects.requireNonNull(resource.getFilename()).toLowerCase().contains(".json"),
          "The file format is incorrect, location: "
              + resource
              + " (please configure a json file or check your Holiday configuration)");
    }
  }
}
