...
public class HolidayFix {
  ...
  public static boolean updateFile() {
    HolidayProperties holidayProperties =
        (HolidayProperties) ApplicationContextUtil.getBean(HolidayProperties.class);
    if ("local".equals(holidayProperties.getType())
        && StringUtils.hasText(holidayProperties.getPath())) {
      if (FileUtil.exist(holidayProperties.getPath())) {
        FileUtil.writeString(
            JSON.toJSONString(ConstantData.getAllDateMap()),
            holidayProperties.getPath(),
      ...
      log.warn(
          "文件更新失败，请检查配置文件，type 或 path 配置错误, type: {}  path: {}",
          holidayProperties.getType(),
          holidayProperties.getPath());
    }
    return false;
  }
